        ...
        router.post('/webhook', (req, res) => {
            ...
            try {
                ...
                if (message.type === 'text') {
                    (0, router_2.handleIncomingMessage)(from, message.text.body).catch((err) => console.error('Message handler error:', err.message));
                }
                else if (message.type === 'audio') {
                    (0, router_2.handleVoiceMessage)(from, message.audio.id).catch((err) => console.error('Voice handler error:', err.message));
                }
                else {
                    (0, router_2.handleIncomingMessage)(from, `[${message.type}]`).catch((err) => console.error('Handler error:', err.message));
                }
            }
            catch (error) {
